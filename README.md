...
esp32-4to20ma-board.kicad_pcb     Board, 40x61mm portrait — parts placed, not routed
...
```
POWER INPUT  (USB-C, ESD, 3.3 V LDO, LED)   ->   MICROCONTROLLER  (ESP32-S3)
24 V LOOP SUPPLY  (MT3608 boost)            ->   4-20 mA LOOP  (limiter, shunt, INA226)
```
...
- **Blocks and notes:** each section is split into titled blocks with dashed
  outlines, and carries its design calculations as on-sheet notes.
...
Components are placed (not routed) on a **40 × 61 mm** two-layer board,
...
- runs DRC and **splits it in two** — placement problems, which are yours to
  ...
  cannot fix. Expect **5** and **5** respectively, plus 118 unconnected items
  until the board is routed, and `schematic parity: clean`;
...
- **The board is placed but not routed.** `place_components.py` has put all 52 footprints on a 40×61mm outline and assigned their nets, so *Update PCB from Schematic* finds nothing to change — what is left is copper. Native USB traces (IO19/IO20 to the connector) should be routed as a short, matched-length differential pair — see the S3 hardware design guidelines if you want true 90Ω impedance control; at USB Full-Speed (12 Mbps) this is good practice but not as strict a requirement as USB 2.0 High-Speed.
